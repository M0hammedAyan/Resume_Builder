...
from app.services.resume_templates import list_resume_templates, normalize_template_id
...
def export_resume(
    ...
    selected_template = normalize_template_id(payload.template_id or resume.selected_template)
    resume_json["selected_template"] = selected_template
    try:
...
def download_resume_pdf(
    ...
    try:
        file_path, download_name = export_resume_pdf(db, user_id=user_id, template_name=template_id)
    except Exception as exc:  # noqa: BLE001
...
def download_resume_docx(
    ...
    try:
        file_path, download_name = export_resume_docx(db, user_id=user_id, template_name=template_id)
    except Exception as exc:  # noqa: BLE001
...
def read_resume_templates() -> list[dict[str, object]]:
